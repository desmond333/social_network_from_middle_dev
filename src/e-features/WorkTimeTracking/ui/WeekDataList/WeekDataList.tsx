...
export const WeekDataList: FC<WeekDataListProps> = (props) => {
  ...
  const [sortByHoursDirection, setSortByHoursDirection] = useState<
    TSort | false
  >('up')
  const [sortByDateDirection, setSortByDateDirection] = useState<TSort | false>(
    false
  )
      ...
      <VerticalOffset offset={'level5'}>
        <BlockWithShadow>
          <Space columnGap={'level2'}>
            ...
            <Button variant={BtnVariant.BACKGROUND} onClick={onSortWeeksByDate}>
              {`Sort by date `}
